...
## PACKAGE TO BUILD
Package=github.com/waltzofpearls/sawmill
...
do
  ...
  if [[ $zipname == *.zip ]]; then
    zip -j $zipname $bin $PackageDir/LICENSE $PackageDir/README.md $PackageDir/config.example.yml
  else
    tar -cvzf $zipname -C $BuildDir $binbase -C $PackageDir LICENSE README.md config.example.yml
  fi
